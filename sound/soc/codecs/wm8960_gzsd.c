...
#include "wm8960_gzsd.h"

#include <linux/slab.h>

#define AUDIO_NAME "wm8960"
#define WM8960_VERSION "gzsd_0.4"
 ...
 */
int wm8960_write(struct snd_soc_codec  *codec, unsigned int reg,
	unsigned int value)
{
	u8 data[2];
...
EXPORT_SYMBOL_GPL(wm8960_write);
//#define GZSD_READ
#ifdef	GZSD_READ
static void gzsd_read(struct snd_soc_codec  *codec)
{
	u16 reg;
	reg = wm8960_read(codec,WM8960_LDAC);
	printk("reg is 0x%x,value is 0x%x",WM8960_LDAC,reg);
	//while(1);
}
#endif
...
static int wm8960_add_controls(struct snd_soc_codec *codec)
{
	...
	for (i = 0; i < ARRAY_SIZE(wm8960_snd_controls); i++) {
		err = snd_ctl_add(codec->card, snd_soc_cnew(&wm8960_snd_controls[i],codec, NULL));
		if (err < 0)
			return err;
	}
...
{
	//static struct wm8960_data wm8960_pdata = gzsd_pdata;
...
static int wm8960_init(struct snd_soc_device* socdev)
{
	...
	reg = wm8960_read(codec, WM8960_LOUTMIX);
	wm8960_write(codec, WM8960_LOUTMIX, reg | 0x100);
	reg = wm8960_read(codec, WM8960_ROUTMIX);
	wm8960_write(codec, WM8960_ROUTMIX, reg | 0x100);

	reg = wm8960_read(codec, WM8960_POWER3);
	wm8960_write(codec, WM8960_POWER3, reg | 0xc);

	reg = wm8960_read(codec, WM8960_LOUT1);
	wm8960_write(codec, WM8960_LOUT1, reg | 0x7f);
	reg = wm8960_read(codec, WM8960_ROUT1);
	wm8960_write(codec, WM8960_ROUT1, reg | 0x7f);

	reg = wm8960_read(codec, WM8960_POWER2);
	wm8960_write(codec, WM8960_POWER2, reg | 0x60);
	/* register pcms */
			    ...
			    const struct i2c_device_id *id)
{
	...
	if (ret < 0)
		dev_err(&i2c->dev, "failed to initialise WM8960\n");
	return ret;
}
...
static int wm8960_add_i2c_device(struct platform_device *pdev,
				 const struct wm8960_setup_data *setup)
{
...
static int wm8960_probe(struct platform_device *pdev)
{
	...
	{
		printk("wm8960_probe: i2c_master_send\n");
		codec->hw_write = (hw_write_t)i2c_master_send;
		ret = wm8960_add_i2c_device(pdev, setup);
		if (ret != 0)
