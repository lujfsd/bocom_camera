...
int gzsd210_adc_acquire_io(void) {
	int ret;

	_adccon = readl(base_addr  + S3C_ADCCON);
	_adctsc = readl(base_addr  + S3C_ADCTSC);
	_adcdly = readl(base_addr  + S3C_ADCDLY);
	
	return ret;
}

void gzsd210_adc_release_io(void) {
	
	writel(_adccon, base_addr  + S3C_ADCCON);
	writel(_adctsc, base_addr  + S3C_ADCTSC);
	writel(_adcdly, base_addr  + S3C_ADCDLY);
	writel(WAIT4INT(0), base_addr  + S3C_ADCTSC);

	
}
#else
static inline int gzsd210_adc_acquire_io(void) {
	return 0;
}
static inline void gzsd210_adc_release_io(void) {
	/* Nothing */
}
#endif
